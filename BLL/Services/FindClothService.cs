        ...
        public List<StoreDTO> FindStores(SearcClothDTO search)
        {
            ...
            try
            {
                ...
                using (db = new storesEntities())
                {
                    ...
                    list = db.Inventories.Where(i => i.ClothID == clothcod).Select(inve => new InventoryDTO() { ClothID=inve.ClothID,StoreID=inve.StoreID,YearOfProduction=inve.YearOfProduction}).ToList<InventoryDTO>();
